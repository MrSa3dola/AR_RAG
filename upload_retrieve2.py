...
INDEX_NAME = "rag-data"
PINECONE_API_KEY = (
    "pcsk_anKYK_Gz8wGefNQBjPdPoVBPmR7GAfMkUvTCgqNjv2gYVJ7op7zofvqWzVkBfSY8qzi56"
)
        ...
        vectors = [
            {
                "id": f"vec_{i+j}",
                "values": emb,
                "metadata": {"caption": cap, "image_path": img, "price": str(prc)},
            }
            for j, (cap, img, prc, emb) in enumerate(
                zip(batch_captions, batch_images, batch_prices, embeddings)
            )
        ]
